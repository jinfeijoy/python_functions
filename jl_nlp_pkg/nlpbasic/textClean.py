...
def doc_tokenize(examples, remove_pattern = [], lower_case = False,
                 deacc = False, encoding = 'utf8', errors = 'strict'):
    """
...
def remove_invalid_tokens(tokenzied_corpus, nltk_stop = True, stop_word_list=[],
...
def get_multi_gram_tokens(corpus, multi_gram):
    '''
   ...
   '''
    ...
    for doc in corpus:
        ...
        for ngram in multi_gram:
            filtered_sentence = []
            n_grams = ngrams(doc, ngram)
            for grams in n_grams:
                filtered_sentence.append(" ".join(grams))
            multi_tokens.extend(filtered_sentence)
...
def pipeline(corpus, multi_gram= [1], nltk_stop = True, stop_word_list = [], remove_pattern = [],
    ...
    result = []
    for doc in corpus:
        tmp_tokens = doc_tokenize(doc, remove_pattern = remove_pattern, lower_case = lower_case,
                     deacc = deacc, encoding = encoding, errors = errors)
        result.append(tmp_tokens)

    result = stem_lemma_process(result, stem_lemma = stem_lemma)
    result = remove_pos_token(result, tag_drop = tag_drop)
    result = remove_invalid_tokens(result, nltk_stop = nltk_stop, stop_word_list=stop_word_list,
                                   check_numbers=check_numbers, word_length=word_length,
                                   remove_consecutives=remove_consecutives)
    result = get_multi_gram_tokens(result, multi_gram = multi_gram)
    return result
